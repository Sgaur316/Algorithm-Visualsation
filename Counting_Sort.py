...
def counting_sort(data, drawData, timeTick):
    ...
    for i in range(n):
        for j in range(count[i]):
            data[k] = i
            drawData(data, [PEACHER for x in range(len(data))])
            time.sleep(timeTick)
            k += 1

    drawData(data, [PINKER for x in range(len(data))])


# Main window
window = Tk()
window.title("Couting Sort Visualizer")
...
def sort():
    ...
    if algo_menu.get() == 'Counting Sort':
